...
def SIMS_cluster(train_x,cell_idx,cell_pos,k,save_path,use_representation_list=['total_mean','median_mean','SIMS_id'],use_algorithm_list=['SC3','SIMLR','UMAP_HDBSCAN'],true_y=None,figsize=(10,10),font_scale=1.5,figformat='svg'):
...
#         再准备distill
        ...
        for rep_idx in range(len(representation_list)):
                ...
                if 'SIMLR' in use_algorithm_list:
...
#         return label_list,CM_list
        ...
        cur_save_file = '{0}ARI_mat_{1}.{2}'.format(rst_path,time_str,figformat)
        print('saving ARI mat to {0}...'.format(cur_save_file))
        sns.set(style="white", font_scale=font_scale)
        sns.clustermap(figsize=figsize,row_cluster=False,data=pd.DataFrame(ari_mat),col_cluster=False,cmap="vlag",square=True,yticklabels=label_name_list,xticklabels=label_name_list)
        ...
        start = timeit.default_timer()
        base_clustering_mat = numpy2mat(np.transpose(np.array(label_list).astype('int')))
...
#         resultsLWEA=eng.compute_LWCM(base_clustering_mat,0.5,matlab.double(consensus_k_range))
        ...
        cur_save_file = '{0}LWEA_{1}.pkl'.format(mid_path,time_str)
        # num_cells*num_labeling
        pickle.dump(resultsLWEA,open(cur_save_file,'wb'))
        stop = timeit.default_timer()
        print('done! Time cost: ',stop-start)
    ...
    # fill LWCM_np diagonal with maxvalue
        ...
        cur_save_file = '{0}LWCM_{1}.{2}'.format(rst_path,time_str,figformat)
        print('saving LWCM to {0}...'.format(cur_save_file))
        sns.set(style="white", font_scale=font_scale)
        sns.clustermap(LWCM_np, row_linkage=linkage_mat,col_linkage=linkage_mat,figsize=figsize,cmap='vlag')
        plt.savefig(cur_save_file,transparent=True,format=figformat,bbox_inches='tight')
        print('done!')
        ...
        cur_save_file = '{0}k_score.{1}'.format(rst_path,figformat)
        print('saving k_score to {0}...'.format(cur_save_file))
        sns.set(style="white", font_scale=font_scale)

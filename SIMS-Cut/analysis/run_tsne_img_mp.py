...
def run_tsne_img(data_name,num_features,path_pref='/home/yzy/bioSIMS/data/process/',bg_threshold=10,figformat='png',figsize=(10,10)):
...
def mp_run_tsne_img(i):
    cur_triple = triple_list[i]
    data_name = cur_triple[0]
    num_features = cur_triple[1]
    bg_threshold = cur_triple[2]
    run_tsne_img(data_name,num_features,path_pref='/home/yzy/bioSIMS/data/process/',bg_threshold=bg_threshold,figformat='eps')
...
#data_name_list=['20190115_liverfiber1_filter','20190115_liverfiber2_filter']
data_name_list=['20190303_liverfiber_fix']
# data_name_list=['20190117_liverfiber1_pos','20190117_liverfiber2_pos']
#num_features_list = [168,165,164,159,202,181]
num_features_list=[183]
# num_features_list=[203,209]
bg_threshold_list = [0,2,4,6,8,10]
triple_list = []
for i in range(len(data_name_list)):
    data_name = data_name_list[i]
    num_features = num_features_list[i]
    for bg_threshold in bg_threshold_list:
        triple_list.append((data_name,num_features,bg_threshold))

#pool = mp.Pool(len(triple_list))
pool = mp.Pool(40)
pool.map(mp_run_tsne_img,range(len(triple_list)))
#num_features_list = [168,165,164,159,202,181]

...
if(use_edges==0)
	if(auto_type==4)
        edge_auto = get_BK_pairwise_ext(test_samples_20,1,sz,sz);
%     save(['edges/edges_ori_auto_',batch_files{i}],'edge_auto');
    save(strcat(cut_edges_path,'edges_ori_auto_',batch_files{i}),'edge_auto');
   edge_ada = adaptive_constrast2_ext(edge_auto,21,sz,sz);
   save(strcat(cut_edges_path,'edges_ori_ada_',batch_files{i}),'edge_ada');
	end

if(auto_type==8)
        edge_auto = get_BK_pairwise_8_autosigma(test_samples_20,1,sz,sz);
%     save(['edges/edges_ori_auto_',batch_files{i}],'edge_auto');
    save(strcat(cut_edges_path,'edges_ori_auto_',batch_files{i}),'edge_auto');
   edge_ada = adaptive_constrast2(edge_auto,21,sz,sz);
   save(strcat(cut_edges_path,'edges_ori_ada_',batch_files{i}),'edge_ada');
        end
...
save(strcat(save_final_labeling_path,'datamat_',num2str(beta),'.mat'),'data_mat');
end
